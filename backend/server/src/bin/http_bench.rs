...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Health: {}/{} succeeded in {:?}",
        health_success, health_ops, elapsed
    );
    if elapsed.as_secs_f64() > 0.0 {
        println!(
            "  → {:.2} ops/s",
            health_success as f64 / elapsed.as_secs_f64()
        );
    }
    ...
    println!(
        "Search: {}/{} succeeded in {:?}",
        search_success, search_ops, elapsed
    );
    if elapsed.as_secs_f64() > 0.0 {
                ...
                if let Some(listings) = json.get("listings").and_then(|l| l.as_array()) {
                    if let Some(first) = listings.first() {
                        if let Some(listing_id) = first.get("listing_id").and_then(|id| id.as_str())
                        {
                            ...
                            let elapsed = start.elapsed();
                            println!(
                                "Get Listing: {}/{} succeeded in {:?}",
                                get_success, get_ops, elapsed
                            );
                            if elapsed.as_secs_f64() > 0.0 {
